...
class GreeksFetcher:
    ...
    def __init__(self, broker_client=None):
        ...
        self.cache_duration = 5  # Cache for 5 seconds (rate limit is 1/sec)
        self.last_api_call = 0
        self.rate_limit_interval = 1.1  # 1.1 seconds between calls
    ...
    def get_expiry_date_str(self) -> str:
        """Get current week's expiry date in API format (e.g., '30JAN2026')"""
        now = datetime.now()
        ...
        if days_ahead < 0:  # Past Thursday this week
            days_ahead += 7
        elif days_ahead == 0 and now.hour >= 15:  # Thursday after 3:30 PM
            days_ahead = 7
    ...
    def fetch_from_api(self, underlying: str = "NIFTY", strike_price: int = None) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"[DEBUG] Greeks API error: {e}")
            return None
