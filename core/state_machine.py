    ...
    if has_signal:
        ...
        if state.consecutive_entry_signals < required_signals:
            if state.loop_count % 100 == 0:
                logger.info(f"🔄 Signal {state.consecutive_entry_signals}/{required_signals}")
            return "IDLE"
    else:
...
def state_entry_ready(tick: Dict, greeks: Dict, state: TradingState,
                      broker, logger) -> str:
    """Handle ENTRY_READY state - Place order"""
    state.consecutive_entry_signals = 0
...
def state_in_trade(tick: Dict, greeks: Dict, state: TradingState,
    ...
    from core.entry_engine import MAX_RECENT_TICKS
