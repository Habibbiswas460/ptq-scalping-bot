...
- **Found but not fixed, out of scope, flagged for a separate decision — correction below in §22:** this entry originally claimed `app.py` "does not appear to be the live entry point," based only on grepping `core/main.py` itself for `prune_old_signal_rows` and finding nothing. That check was incomplete — `run.sh` actually launches `app.py` (`"$PYTHON_BIN" app.py`, `run.sh:773`), which prunes once at startup and then calls `core.main.run_with_auto_reconnect()` (the function that prints the "── PTQ SCALPING BOT ── Auto-Reconnect ON ──" banner seen in every session's logs). So pruning was already running in the live path once per process start, contrary to what this entry said. See §22 for the actual fix and the correction.
...
## 22. `prune_old_signal_rows()` wired directly into `core/main.py:main()`, and §21's mistaken claim corrected (2026-09-02)

**Correction to §21 first:** §21 claimed pruning "may never actually get pruned in the process that's actually running," based only on grepping `core/main.py` for `prune_old_signal_rows` and finding nothing. That check was incomplete. `run.sh` launches `app.py` directly (`"$PYTHON_BIN" app.py`, `run.sh:773`), and `app.py` already does `prune_old_signal_rows(retention_days=7)` once at startup, before calling `core.main.run_with_auto_reconnect()` — the function whose `"── PTQ SCALPING BOT ── Auto-Reconnect ON ──"` banner appears in every session's logs analyzed in this investigation, confirming it's genuinely the live path. Pruning was already running in production, once per process start. The earlier claim was wrong; correcting the record rather than leaving it standing.

**What was still worth doing anyway:** `main()` — not `run_with_auto_reconnect()` or `app.py` — is the function actually re-invoked on every auto-reconnect, and is also directly callable on its own (as `tests/test_p0_historical_warmup.py` already does, calling `core_main.main()` without going through `app.py` at all). Wiring pruning directly into `main()`, right after the config-validation block added in §18.2, means it runs regardless of entry point rather than depending on `app.py` specifically remaining the sole launcher — the same defense-in-depth reasoning already applied to that config-validation call.

**Fix:** added a pruning call at the top of `core/main.py:main()`, wrapped in the same `try/except Exception: pass` `app.py` already uses (pruning must never block startup). Runs on every `main()` call including reconnects, not just once per process — acceptable since it's a cheap, idempotent `DELETE WHERE timestamp < cutoff`, same reasoning already applied to the config-validation call re-running on reconnect too.

**Tests:** `tests/test_main_startup_pruning.py` (2 tests) — `main()` calls `prune_old_signal_rows(retention_days=7)` before reaching the (mocked-to-abort) readiness check, and a pruning failure doesn't prevent `main()` from continuing past it.
